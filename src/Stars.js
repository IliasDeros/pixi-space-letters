// Source: https://codepen.io/shableep/pen/RWZmrg
...
/**/
export function Stars({ app, callback }) {
  ...
  this.newStar = function () {
    ...
    const canvas = this.getStarGraphic();
    var texture = PIXI.Texture.from(canvas);
    var star = new PIXI.Sprite(texture);
    var scale = 0.2 + this.starSizeDistribution(Math.random()) * 0.5;
    ...
    star.fadeSpeed =
      settings.fadeSpeedMin + settings.fadeSpeedMax * Math.random();
    ...
    if (star.alpha < settings.alphaMin)
      star.fadeSpeed = Math.abs(star.fadeSpeed);
    if (star.alpha >= settings.alphaMax)
      star.fadeSpeed = -Math.abs(star.fadeSpeed);
